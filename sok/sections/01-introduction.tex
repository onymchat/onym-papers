...
\begin{itemize}
  ...
  \item For the Stellar~+~BLS12-381~+~PLONK row, we run a
    first-party benchmark harness (\S\ref{sec:analysis},
    appendix). The C1 deployment under measurement consists of five
    Soroban contracts (one per policy) deriving all of their
    verifying keys from a single trusted-setup parameter --- the
    Ethereum Foundation KZG Summoning
    Ceremony~\cite{eth-kzg-ceremony} --- whose provenance is
    hash-chained from the published transcript through to each
    contract's WASM bytecode (\S\ref{sec:cfg-c1}). The harness
    measures each contract independently and the C1 row aggregates
    them. One row of every comparison table is therefore
