...
empirically-quantified host-function-economically-blocked
configuration (C7) form a partial order, not a total one. The C1
and C7 rows are first-party measured on Stellar testnet; C7's
deploy-cost multiplier of $\sim$590$\times$ over C1 (with
proof-bearing entrypoints additionally tooling-blocked on a PCS
layer) is the load-bearing empirical refinement to the
host-function-axis discussion. The deployment recommendations of
\S\ref{sec:recommendations} are accordingly profile-conditional,
and two of the four profiles point to configurations that would not
be selected under the profile-blind reading of the comparison
tables.

For protocol designers, the cross-axis dependency analysis of
\S\ref{sec:dimensions-deps} is the actionable contribution:
fixing the anchor chain for non-cryptographic reasons --- team
familiarity, regulatory posture, fee economics --- implicitly fixes
four of the six axes, leaving only the in-circuit hash truly free.
Designers who treat the chain choice as independent of the
proof-system choice underestimate the constraint they have already
accepted.

For the cryptographic-research community, the migration matrix of
\S\ref{sec:migration-matrix} and the open-problem cluster of
\S\ref{sec:open} together identify the gap: among PQ-ward
destinations, no column of the matrix is uniformly Class~(a), and
the empty cheap-PQ-ready-and-decentralized cell of
