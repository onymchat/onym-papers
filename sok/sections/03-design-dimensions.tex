...
(constructed within the Barreto--Lynn--Scott
family~\cite{barreto-2003-bls}),
BN254~\cite{barreto-2005-bn254} (with the security re-evaluation
of~\cite{kim-barbulescu-2016} as a security-relevant footnote), the
...
\begin{enumerate}
  \item \textbf{Native precompile / host function}: the chain exposes
    ...
    Ethereum's BN254 precompile (EIP-197); Stellar's BLS12-381 host
    functions added in Protocol 22~\cite{stellar-cap-0059}, BN254
    host functions added in Protocol 25~\cite{stellar-cap-0074},
    and Poseidon / Poseidon2 permutation primitives also added in
    Protocol 25~\cite{stellar-cap-0075}; Mina's built-in Pickles
    verifier.
  ...
  \item \textbf{Verifier-as-rollup}: the chain itself is a SNARK-rollup
    ...
    chain. Aztec is the canonical example. Configurations in this
    regime collapse three axes (anchor chain, verification host,
    SNARK system) into a single bound choice; the six-axis taxonomy
    treats the C3 row as a pre-bundled point rather than as freely
    varying along all six.\footnote{The collapse is structural rather
    than accidental: a SNARK-rollup whose execution model is its
    verifier cannot independently vary the SNARK without
    re-architecting the rollup. The taxonomy retains C3 as a
    distinct row because its operational properties --- proof
    aggregation, sequencer trust, verifier-as-execution semantics
    --- are not reducible to any of the other six configurations
    surveyed.}
\end{enumerate}

This axis is the binding economic constraint that
shapes the ``Stellar~+~hash-based'' configuration C7 in
\S\ref{sec:configurations} and \S\ref{sec:cfg-c7}. Stellar's
host-function set covers pairings on BLS12-381 and BN254 and
Poseidon-arithmetization permutations~\cite{stellar-cap-0075}, but
not the FRI stack: CAP-0075's rationale explicitly scopes out NTT,
Goldilocks-field arithmetic, and FRI primitives. A transparent-setup
hash-based verifier on Stellar therefore runs in pure WASM for its
FRI-specific operations, including Merkle-authentication-path
traversal and the low-degree-test fold loop. The empirical
deploy-cost ratio of $\sim$590$\times$ between C7 and C1
(\S\ref{sec:cfg-c7}) is the load-bearing measurement: the
host-function gap is not a binary deployable / undeployable
classification but an economic gradient that biases sharply against
hash-based deployments. The same constraint forces Ethereum L1
deployments toward BN254 over BLS12-381 in the absence of EIP-2537.
...
\input{figures/design-space-axes}
