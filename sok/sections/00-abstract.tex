...
across five comparison dimensions. The
Stellar~+~BLS12-381~+~PLONK configuration --- a five-contract
deployment whose trusted setup is reused from the Ethereum
Foundation KZG ceremony --- is measured by a first-party benchmark
harness whose methodology is documented in the paper's source
repository; all other configurations cite published
benchmarks date-stamped to 2026-04. Our central finding is that no
configuration dominates: four deployment profiles ---
cost-optimized, post-quantum-prioritized, censorship-resistance-prioritized,
and jurisdiction-constrained --- each select a different point in the
...
as ready, blocked on chain host functions, blocked on tooling maturity,
or open in cryptographic construction; the
cheap-PQ-ready-and-decentralized cell of the design space is currently
empty.
\end{abstract}
