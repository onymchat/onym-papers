...
\begin{description}
  \item[\textsc{C1 $\to$ C7} (commitment-scheme swap, PLONK
    preserved):] retain the Stellar anchor and the PLONK polynomial
    IOP, swap the KZG polynomial-commitment scheme for FRI
    (the Plonky3 design). The relation circuits are unchanged; only
    the commitment-scheme layer is replaced. Class \textbf{(b)}:
    Plonky3 is mature enough for production use, but Stellar's
    host-function set --- BLS12-381 pairings (Protocol
    22~\cite{stellar-cap-0059}), BN254 pairings (Protocol
    ...
    \S\ref{sec:open-host}. C1's PLONK choice (rather than Groth16)
    makes this migration simpler than the corresponding migration
    from a Groth16 starting point would be: the relation circuits
    do not need to be re-arithmetized for a different proof-system
    family.
