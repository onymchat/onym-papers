...
$\Verify(\mathsf{vk},
(\Cg, \Cgp_{\star}, e, \texttt{sep-anarchy}), \pi_{\star}) = 1$
...
\in \RUpd^{\sep{anarchy}}$. The lemma's quantification covers
...
The predicate of $\RUpd^{\sep{anarchy}}$ then gives:
\begin{enumerate}
\item $\ell_a \ne \Empty$ and
...
$(\Cg, \Cgp_{\star}, e, \texttt{sep-anarchy}; w_{\star})$ in
$\RUpd^{\sep{anarchy}}$ --- contradicting the win condition of
...
over without re-derivation. Richer policies introduce
additional in-circuit invariants: for example,
$\sep{democracy}$ adds a quorum predicate and may require
nullifiers to prevent vote double-counting; $\sep{oligarchy}$
requires founder-set membership and a fixed-cardinality check;
$\sep{oneonone}$ requires two co-signatures and an
anti-double-use constraint; $\sep{tyranny}$ requires an
admin-key check; policies that admit removal or rotation
require revocation invariants and consistent role bookkeeping.
Each such invariant changes the witness shape of $\RUpd$ and
therefore changes the post-extraction reasoning that connects a
satisfying witness to a permitted state transition. A
soundness theorem for any such policy must redo the
state-transition enumeration; the reusable content is the
cryptographic stack beneath, not the predicate-specific
analysis above it.
\end{corollary}
