...
transform, and the PLONK proof system --- with each layer
contributing a single concrete soundness reduction. We make every
reduction explicit and trace the contract's authorisation claim
down to four base assumptions: the $d$-strong Bilinear
Diffie--Hellman ($d$-SBDH) problem on BLS12-381, collision
resistance of the Poseidon2 permutation under its published
parameters, the random-oracle modelling of Poseidon2 inside the
Fiat--Shamir transcript, and the one-honest-contributor assumption
of the reused structured reference string. The result is a
self-contained checklist of what must hold for the contract to be
sound, and a precise statement of what fails first under a
...
\begin{aligned}
   &t = \texttt{sep-anarchy} \\
   &w = (\ell, \mathsf{path}, \ell^{\star}, e_{\mathrm{prev}}) \\
   &\mathsf{MerkleVerify}(\Cg, \ell, \mathsf{path}) = 1 \\
   &\Cgp = \mathsf{Insert}(\Cg, \mathsf{path}, \ell^{\star}) \\
   &e = e_{\mathrm{prev}} + 1
\end{aligned}\right\}
\]
where $(\Cg, \Cgp, e, t)$ is the public input, $w$ is the
witness, $\ell^{\star}$ is the new leaf, and $\mathsf{Insert}$ is
the deterministic Merkle-tree insertion routine that rehashes the
sibling path with $\ell^{\star}$ in place of an empty slot.
\end{definition}
...
the current epoch. The contract is \emph{authorisation-sound} if,
for every probabilistic polynomial-time adversary $\Adv$,
\[
\Pr\!\left[
\begin{aligned}
&\Verify(\mathsf{vk}, (\Cg, \Cgp_{\star}, e\!+\!1, \texttt{sep-anarchy}), \pi_{\star}) = 1 \\
&\wedge\; \nexists\, w :\; \bigl(\Cg, \Cgp_{\star}, e\!+\!1, \texttt{sep-anarchy}; w\bigr) \in \RUpd^{\sep{anarchy}}
\end{aligned}
...
\begin{lemma}[PLONK knowledge soundness~\cite{gabizon-2019-plonk}]
\label{lem:plonk}
Under \Cref{ass:sbdh} (which implies \Cref{lem:kzg}) and
\Cref{lem:fs}, in the algebraic group model, the PLONK proof
system has knowledge soundness against probabilistic
polynomial-time adversaries. That is, for every PPT adversary
$\Adv$ producing an accepting proof, there exists a PPT extractor
$\mathcal{E}^{\Adv}$ that, given $\Adv$'s code and random tape,
outputs a witness $w$ such that the public input together with
$w$ satisfies the circuit.
\end{lemma}
...
\Cref{lem:plonk}, although no such separation is currently known
that would apply to the structures used by PLONK.
\end{remark}
...
game with non-negligible probability $\varepsilon$. By
\Cref{lem:plonk}, an extractor $\mathcal{E}^{\Adv}$ produces, with
probability at least $\varepsilon - \negl(\lambda)$, a witness
$w_{\star} = (\ell, \mathsf{path}, \ell^{\star}, e_{\mathrm{prev}})$
such that
$(\Cg, \Cgp_{\star}, e + 1, \texttt{sep-anarchy}; w_{\star})
\in \RUpd^{\sep{anarchy}}$ (see \Cref{def:rupd}).

By the policy predicate of $\RUpd^{\sep{anarchy}}$, the witness
contains a Merkle authentication path $\mathsf{path}$ such that
$\mathsf{MerkleVerify}(\Cg, \ell, \mathsf{path}) = 1$, and
$\Cgp_{\star} = \mathsf{Insert}(\Cg, \mathsf{path}, \ell^{\star})$.
By \Cref{lem:merkle}, the path is unique up to the leaf it
opens, so $\ell$ is determined and lies in the membership set
committed to by $\Cg$. Therefore $\Cgp_{\star}$ is the result of
inserting $\ell^{\star}$ under a valid current-member witness ---
which contradicts the win condition of \Cref{def:auth}.

Hence $\varepsilon = \negl(\lambda)$, modulo the negligible
losses from the three reductions in
\Cref{lem:plonk,lem:fs,lem:merkle,lem:kzg}.
\end{proof}
...
soundness (\Cref{lem:plonk}), then \Cref{thm:auth}. The stack
...
(\Cref{lem:plonk}) has already failed.
\end{proposition}
...
predicate's content does not enter \Cref{lem:plonk} or any layer
...
reused structured reference string. We made explicit which
assumption fails first under a CRQC, and we listed the
operational practices the proof is conditional on. The result is
a checklist of what must hold for the deployment to be
authorisation-sound, and a precise statement of the boundary
between the part of the stack that survives Shor and the part
that does not.
